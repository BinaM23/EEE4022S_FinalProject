...
%Run 1000 times to simulate 1000 users gain calculation
%NB each user will have different network conditions
%evaluates all possible combinations to 1dp

%Update 20/10/2023
%Updated range of values for bandwidth 

%0.1,0.8,0.1 was the best combination for all users in both fixed and
...
%comment out when doing the dynamic network experiment
%Defining the RATs
...
%user specified weights are random for each criterion
...
%Old method not used anymore, use FAHP_EAM.m instead
...
for n=1:length(values)
    for m=1:length(values)
        for o=1:length(values)
            ...
            if ((num1+num2+num3)==1 && (num1~=1 && num2~=1 && num3~=1) && (num1~=0 && num2~=0 && num3~=0)) %only find gain if the 3 numbers add up to 1 and none of them are 1
