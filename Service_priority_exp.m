...
%UPDATE
%20/10/2023 - updated TOPSIS eqns and range for bandwidth values
%23/10/2023 - Added the extent analysis method of FAHP for Ws

%Experiment was conducted twice, 1st time using static network conditions
%Repeated using dynamic network conditions

%weight proportion params used were based on experiment 1 results
%a=0.1, b=0.8, y=0.1 or a=0.2, b=0.5, y=0.3
...
%Uncomment if you want to see closeup results for each scenario
%mybar=bar(mydata1);
%xlabel("RAT-N");
...
DATA=[mydata1;mydata2;mydata3;mydata4;mydata5;mydata6;mydata7;mydata8;mydata9;mydata10];
x=1:10;
bar(x,DATA)
xlabel("Scenario")
...
for u = 1:num(2) %calculate score for all u users
    ...
    for i=1:4
        ...
        tally2=tally2+(D(i,j)-Dminus(j))^2; %sum ""  (22)
        end
...
%comment this out when doing dynamic experiment
%3G
...
%to use this method comment out extent analysis then uncomment below code
...
%%uncomment out to use 2018 method weights
...
%======================================================================================================================
%UPDATE 23/10/2023
%Extent analysis method to get Ws
%Program to workout FAHP by extent analysis method
%to use this method comment out above method then uncomment this section
...
%Fuzzy comparison matrices per service
...
%uncomment to use Extent analysis method
wSvoice=zeros(1,4);
wSvideo=zeros(1,4);
wSweb=zeros(1,4); %vectors to store calculated weights of criteria per service
...
inverseTFN=[1/colsum(1),1/colsum(2),1/colsum(3)]; %take reciprocal
%calculate Si values
Si_values=cell(1,4);
for k=1:4
...
wSweb;
%uncomment out below section to use extent analysis weights
wsnormvec=[wSvoice;wSvideo;wSweb]; %each row is the normalized weight vector for each service
Ws=zeros(1,4);
total=0;
for j=1:4
...
%a=0.2;b=0.5;y=0.3; %updated based on experiment 1 results
a=0.1;b=0.8;y=0.1;
ObjectiveWeights;
Ws;
for i=1:N
...
Dvec{kw}=D; %assigns decision matrix to user k
end
