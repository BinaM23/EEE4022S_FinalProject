%Experiment 1 determining alpha,beta and gamma
%Author: Bina E Mukuyamba
%Date: 17/10/2023

%Updated network attributes for BW

%result: so far combination 1 gave highest mean gain
...
%1st experiment:network attributes were kept constant only user specified weights were
%varied for each user so only WU will be different
%repeat of experiment used varied network attributes, therefore only Ws
%stayed constant, everything else changed

%comment out when doing the dynamic experiment
%Defining the RATs
%Default values from Table 3
...
%uncomment to check that each user has 3 sets of weights 
%Assume 1st vector=voice, 2nd vector=voice, 3rd vector=web
%for i=1:numUsers 
%disp(users{i})
%end
...
Ps=Pu; %service determined priority=user specified priority

%normalizing weights and priority
NormedUweights=Normalize(users);

%Display normalized weights for each service specified by user
...
Ps_norm=Pu_norm;%normalized priority vectors
...
%=============================================================================
%Old method not used anymore, used extent anaylsis instead
...
%Ws
%=======================================================================
%Extent analysis method result 
%run FAHP_EAM.m with Pu=[3,3,3] then sub in result
...
%a=0.2;b=0.5;y=0.3; %varied by the experiment
%ObjectiveWeights;
%Ws;
%for i=1:numUsers
...
%EXPERIMENT 1: Varying weight proportion parameters
%tested the same combination of parameters as table 4 in [11]
%function used find_gain(Umat,alpha,beta,gamma,WU,WS,WO)
%Experiment was done twice using default network conditions/attribute
%values
%2nd time randomly varied network Attribute values (Mt) for each iteration
%user allocated weights were random and priority was constant
%gains-i corresponds to the calculated gains for combination i
%i=1...7
gains1=zeros(1,numUsers); %vectors to store gains for all users for a particular parameter combination
gains2=zeros(1,numUsers); %gains for combination 2 from table 4 in [11] 
gains3=zeros(1,numUsers);
gains4=zeros(1,numUsers);
gains5=zeros(1,numUsers);
gains6=zeros(1,numUsers);
gains7=zeros(1,numUsers); %and so on up to combination 7

for i=1:numUsers
    %there will be numUsers*7 gains in total, each user will have 7 gain
    %values, 1 for each combination
    WU=Wu_allUsers{i}; %user weight preference vector is unique for each user
    gains1(i)=find_gain(U,0.2,0.3,0.5,WU,Ws,ObjectiveWeights); %gain for ith user for combination i
    
    gains2(i)=find_gain(U,0.3,0.2,0.5,WU,Ws,ObjectiveWeights);

    gains3(i)=find_gain(U,0.2,0.5,0.3,WU,Ws,ObjectiveWeights);

    gains4(i)=find_gain(U,0.3,0.5,0.2,WU,Ws,ObjectiveWeights);

    gains5(i)=find_gain(U,0.5,0.2,0.3,WU,Ws,ObjectiveWeights);

    gains6(i)=find_gain(U,0.5,0.3,0.2,WU,Ws,ObjectiveWeights);

    gains7(i)=find_gain(U,(1/3),(1/3),(1/3),WU,Ws,ObjectiveWeights);

end
%store the mean(average) gain of each combiantion into an array
%each position in the array corresponds to a particular scenario
%the combination that has the highest average gain is the one that will be
%used to calculate the comprehensive weights W in the later experiments 
mean_gains=[mean(gains1),mean(gains2),mean(gains3),mean(gains4),mean(gains5),mean(gains6),mean(gains7)]
highest_gain=0;
for g=1:length(mean_gains)
    if(mean_gains(g)>highest_gain)
        highest_gain=mean_gains(g);
        bestParamcombo=g;
    end
end
bestParamcombo
...
%calculates the gain defined by (30) in [11]
%returns a single value (gain)
%inputs: U, a,b,y and Wu,Ws and Wo vectors
